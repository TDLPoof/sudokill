...
public class GroundedPathFinder : MonoBehaviour
{
    ...
    private void OnCollisionStay(Collision collision)
    {
        ...
        if (stepTimer > stepCooldown && collision.gameObject.layer == 0
         && collision.gameObject.tag != "EnemyCOll" && Mathf.Abs(normal.y) < 0.01f)
        {
            transform.Translate(new Vector3(0, stepHeight, 0));
            stepTimer = 0;
        }
    }
